...
class Pipeline {
  ...
  private:
    WorkspaceData build_root_workspace(const RootEntry& root) const {
        ...
        spdlog::info("  {} project(s), {} active",
                     build_output.projects.size(), build_output.active_project_count());

        return build_output.to_workspace(cfg_.compiler_arguments_json(), cfg_.options());
    }

    Config cfg_;
    const GradleRunner& runner_; // NOLINT(cppcoreguidelines-avoid-const-or-ref-data-members)
};
